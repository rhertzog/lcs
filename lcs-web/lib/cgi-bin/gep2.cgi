...
#	or (! $ok{'f_gro'} and $createCours))
  ...
  print
    ...
    "<a href=\"http://$hostname/$webDir/result.$pid.html\">",
    "http://$hostname/$webDir/result.$pid.html</a></div>\n",
...
sub traitement_fond {
  ...
  while (1) {
    ...
    if (! -f '/tmp/EnCours.temp') {
      ...
      &entete(RES);
      print RES
	"<strong>Le traitement préparatoire s'est terminé avec succés.</strong><br>";
      last;
    }
  }
  ...
  if (-f '/tmp/f_men.temp') {
	...
	if (($res->entries)[0]) {
 	...
 	# Remplissage et/ou création du PosixGroup Matiere
  ...
  system ("/usr/bin/lynx --dump $documentRoot/$webDir/result.$$.html | mail admin -s 'Importation Texte'");
