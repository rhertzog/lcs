...
if (count($_POST)>0  || count($_GET)>0) {
  ...
  if (count($_POST)>0 ) {
      $vides_gr=$purifier->purifyArray($_POST['vides_gr']);
      $valider=$purifier->purify($_POST['valider']);
        }
  if (count($_GET)>0) $phase=$purifier->purify($_GET['phase']);
}
...
if (is_admin("lcs_is_admin",$login)=="Y") {
    ...
    if (isset ($valider)) {
		for ($loop=0; $loop < count($vides_gr); $loop++) {
			exec ("$scriptsbinpath/groupDel.pl ". escapeshellarg($vides_gr[$loop]),$AllOutPut,$ReturnValue);
	    	if ($ReturnValue == "0") {
    ...
    if (!isset ($valider)) {
		if ( $phase != 1 ) {
	    	...
	    	echo "<meta HTTP-EQUIV=\"Refresh\" CONTENT=\"3;url='".$_SERVER['PHP_SELF']."?phase=1&jeton=".md5($_SESSION['token'].htmlentities($_SERVER['PHP_SELF']))."'\">\n";
	    	echo "      </HEAD>\n";
		 	echo "      <BODY>\n";
	    	echo $phase;
