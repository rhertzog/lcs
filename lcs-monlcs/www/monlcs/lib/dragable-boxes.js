	...
	*/	
	...
	function closeDragableBox(e,inputObj)
	{
		...
		//requete ajax qui supp le rss de la base user
	...
	function showRSSData(ajaxIndex,boxIndex)
	{
		...
		var le_titre =  headerTokens[0];
		if (le_titre.length > 40)
			le_titre= le_titre.substring(0,40)+' ... ';
		document.getElementById('dragableBoxHeader_txt' + boxIndex).innerHTML = '<span>' + le_titre + '&nbsp;<\/span><span class="rssNumberOfItems">(' + headerTokens[1] + ')<\/span>';	// title
		...
		for(var no=1;no<tokens.length;no++){	// Looping through RSS items
			...
			string = string + '<tr><td><img src="' + src_smallRightArrow + '"></td><td><p class=\"boxItemHeader\"><a class=\"boxItemHeader\" href="' + itemTokens[3] + '" onclick="var w = window.open(this.href);return false">' + itemTokens[0] + '<\/a><\/p><\/td><\/tr>';		
		}
		string = string + '<\/table>';
