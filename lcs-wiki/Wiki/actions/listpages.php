...
// tree display
{
    ...
    if ($sort != 'tag')
    {
        ...
        {
            echo '<em><strong>Erreur ActionListPages</strong>: La page ' . htmlspecialchars($tree,ENT_COMPAT | ENT_HTML401 | ENT_SUBSTITUTE, "ISO8859-1") . ' n\'existe pas !</strong>';
            return;
        }
        $links[$tree] = $rootData;
    }
    ...
    foreach ($pages as $page)
    {
        ...
        if ($sort == 'user' || $sort == 'time')
        {
            ...
            if ($sort == 'user' || ($user && $sort == 'time'))
            {
                ...
                {
                    echo htmlspecialchars($page['user'],ENT_COMPAT | ENT_HTML401 | ENT_SUBSTITUTE, "ISO8859-1");
                }
            }
        }
        echo "</li>\n";
