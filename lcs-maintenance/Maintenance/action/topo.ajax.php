...
// onchange	
		...
		if (isset($_POST["bat"]) )			$Bat		=	$_POST["bat"];
		  ...
		  <!-- Etage -->
					<td class="tableau">
						...
						<select name="etage" id="etage" >
							...
							else $result = @mysql_query("SELECT etage from topologie WHERE batiment='$batiment[0]' ORDER BY etage ASC");
							if ($result) {
								echo "<option value=\"\"> -- Choisir -- </option>";
								while ($r = @mysql_fetch_array($result)) {
					...
					<label for="salle">Salle :</label>&nbsp;
						<select name="salle"  id="salle" ><!-- onChange="location = this.options[this.selectedIndex].value;" :: Inutile -->
							...
							if (isset( $Etage ) ) {
								...
								if(!$result) 
									$result = @mysql_query("SELECT id, salle from topologie WHERE batiment='$batiment[0]' AND etage='$etage_[0]'  ORDER BY salle ASC");
							}
							else $result = @mysql_query("SELECT id, salle from topologie WHERE batiment='$batiment[0]' AND etage='$etage_[0]'  ORDER BY salle ASC");
