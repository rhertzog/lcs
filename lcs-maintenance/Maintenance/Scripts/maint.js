...
$(function(){
 ...
 /*gestion du contenu des selects
  * by misterphi
  *
  * fonction appelée avec un settimeout après un load_topo
  */
function myselect () {
...
// à la sélection d un batiment dans la liste
 $batiments.change( function() {
     var jet=$('#jet').val();//jeton de secu
     var val = $(this).val(); // on récupère le nom du batiment
     if(val != '') {
         $etages.empty(); // on vide la liste des etages
         $salles.empty();
             $.ajax({
             url: 'action/requete_topo.php',
             type : 'POST',
             data: 'id_batiment='+ val+'&jeton='+jet,
             dataType: 'json',
             success: function(json) {
                 $etages.append('<option value=""> -- Choisir -- </option>');
                 $.each(json, function(index, value) {
                     $etages.append('<option value="'+ index +'">'+ value +'</option>');
                 });
             }
         });
     }
     });
//a la selection dun etage
$etages.change( function() {
     var val2 = $(this).val(); // on récupère le nom de l'etage
     var jet=$('#jet').val(); //jeton de secu
     var val = $batiments.val();
     if(val != '') {
         $salles.empty(); // on vide la liste des salles
             $.ajax({
             url: 'action/requete_topo.php',
             type : 'POST',
             data: 'id_batiment='+ val+'&id_etage='+val2+'&jeton='+jet,
             dataType: 'json',
             success: function(json) {
                 $salles.append('<option value=""> -- Choisir -- </option>');
                 $.each(json, function(index, value) {
                 $salles.append('<option value="'+ index +'">'+ value +'</option>');
                 });
             }
         });
     }
