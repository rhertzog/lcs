...
public static function DB_ajouter_devoir($prof_id,$groupe_id,$date_mysql,$info,$date_visible_mysql,$date_autoeval_mysql,$doc_sujet,$doc_corrige,$tab_id_profs=array())
{
	...
	$DB_VAR = array(':prof_id'=>$prof_id,':groupe_id'=>$groupe_id,':devoir_date'=>$date_mysql,':devoir_info'=>$info,':devoir_visible_date'=>$date_visible_mysql,':devoir_autoeval_date'=>$date_autoeval_mysql,':devoir_partage'=>$listing_id_profs,':devoir_doc_sujet'=>$doc_sujet,':devoir_doc_corrige'=>$doc_corrige);
	DB::query(SACOCHE_STRUCTURE_BD_NAME , $DB_SQL , $DB_VAR);
...
public static function DB_modifier_devoir($devoir_id,$prof_id,$date_mysql,$info,$date_visible_mysql,$date_autoeval_mysql,$doc_sujet,$doc_corrige,$tab_id_profs=array())
{
	...
	$DB_VAR = array(':date'=>$date_mysql,':devoir_info'=>$info,':visible_date'=>$date_visible_mysql,':autoeval_date'=>$date_autoeval_mysql,':devoir_partage'=>$listing_id_profs,':devoir_doc_sujet'=>$doc_sujet,':devoir_doc_corrige'=>$doc_corrige,':devoir_id'=>$devoir_id,':prof_id'=>$prof_id);
	DB::query(SACOCHE_STRUCTURE_BD_NAME , $DB_SQL , $DB_VAR);
	// sacoche_saisie (maj)
...
public static function DB_modifier_liaison_user_groupe_par_prof($groupe_id,$tab_eleves,$tab_profs,$mode,$devoir_id)
{
	...
	if($mode!='creer')
	{
		...
		$DB_SQL.= 'LEFT JOIN sacoche_user USING(user_id) ';
		$DB_SQL.= 'WHERE groupe_id=:groupe_id ';
		$DB_SQL.= ($devoir_id) ? 'AND user_profil=:profil ' : 'AND user_id!=:prof_id ' ; // Pour [1] on ne s'intéresse qu'aux élèves ; pour [2] on s'intéresse à tout le monde sauf au prof responsable du groupe (non transmis)
		
		$DB_SQL.= 'GROUP BY groupe_id';
		$DB_VAR = array(':groupe_id'=>$groupe_id,':profil'=>'eleve',':prof_id'=>$_SESSION['USER_ID']);
