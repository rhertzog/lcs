...
class DB_Manager {
  ...
  private function __construct(){
    // init de $eol
    if (!isset($_SERVER['HTTP_HOST'])) { // $_SERVER['TERM'] ou $_ENV['SHELL'] existent en cli mais pas en cron
      self::$eol = "\n";
    }
  }
    ...
    if($pool != "" & $dbname != ""){
      try{
        ...
        if($critical==true) {
          ...
          die("Connexion BDD impossible. ".$e->getMessage() .self::$eol);
        } else {
          throw $e;
        }
      }
...
class DatabaseException extends Exception {
  ...
  public function warn() {
		...
		trigger_error($this->_getUserMessage(), E_USER_WARNING);
		restore_error_handler();
  }
   
  public function log() {
  		error_log($this->_getUserMessage());
  }
  ...
  private function _getUserTrace(){
  		...
  		if(!empty($completTrace)) {
			foreach($completTrace as $trace) {
				if(!empty($trace['class']) && substr($trace['class'], 0, 3)==='DB_') continue;
				else return $trace;
			}
		}
		return false;  		
  ...
  private function _getUserMessage(){
  		$message = $this->message;
  		if($userTrace = $this->_getUserTrace()) {
