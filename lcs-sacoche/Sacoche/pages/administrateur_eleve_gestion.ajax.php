...
if( ($action=='ajouter') && $nom && $prenom && $password )
{
	...
	if($sconet_id)
	{
		if( DB_STRUCTURE_ADMINISTRATEUR::DB_tester_utilisateur_identifiant('sconet_id',$sconet_id,NULL,'eleve') )
		{
	...
	if($sconet_num)
	{
		if( DB_STRUCTURE_ADMINISTRATEUR::DB_tester_utilisateur_identifiant('sconet_elenoet',$sconet_num,NULL,'eleve') )
		{
	...
	if($reference)
	{
		if( DB_STRUCTURE_ADMINISTRATEUR::DB_tester_utilisateur_identifiant('reference',$reference,NULL,'eleve') )
		{
	...
	$user_id = DB_STRUCTURE_COMMUN::DB_ajouter_utilisateur($sconet_id,$sconet_num,$reference,'eleve',$nom,$prenom,$login,crypter_mdp($password),0,$id_ent,$id_gepi);
...
if( ($action=='modifier') && $id && $nom && $prenom && $login && ( $not_new_mdp || $password ) && ( $not_exit || $sortie_date ) )
{
	// Vérifier que l'identifiant ENT est disponible (parmi tout le personnel de l'établissement)
	if($id_ent)
	{
	...
	if($sconet_id)
	{
		if( DB_STRUCTURE_ADMINISTRATEUR::DB_tester_utilisateur_identifiant('sconet_id',$sconet_id,$id,'eleve') )
		{
			exit('Erreur : identifiant Sconet déjà utilisé !');
		}
	}
	...
	if($sconet_num)
	{
		if( DB_STRUCTURE_ADMINISTRATEUR::DB_tester_utilisateur_identifiant('sconet_elenoet',$sconet_num,$id,'eleve') )
		{
			exit('Erreur : n° Sconet déjà utilisé !');
		}
	}
	...
	if($reference)
	{
		if( DB_STRUCTURE_ADMINISTRATEUR::DB_tester_utilisateur_identifiant('reference',$reference,$id,'eleve') )
		{
			exit('Erreur : référence déjà utilisée !');
		}
	}
	...
	$tab_donnees = array(':sconet_id'=>$sconet_id,':sconet_num'=>$sconet_num,':reference'=>$reference,':nom'=>$nom,':prenom'=>$prenom,':login'=>$login,':id_ent'=>$id_ent,':id_gepi'=>$id_gepi,':sortie_date'=>$sortie_date_mysql);
	if(!$not_new_mdp)
	{
