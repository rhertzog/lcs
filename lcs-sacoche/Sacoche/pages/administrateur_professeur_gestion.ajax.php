...
if( ($action=='ajouter') && $nom && $prenom )
{
	...
	if($sconet_id)
	{
		if( DB_STRUCTURE_tester_utilisateur_SconetId($sconet_id,'professeur') )
		{
	...
	$user_id = DB_STRUCTURE_ajouter_utilisateur($sconet_id,$sconet_num=0,$reference,'professeur',$nom,$prenom,$login,$password,0,$id_ent,$id_gepi);
	// Afficher le retour
...
else if( ($action=='modifier') && $id && $nom && $prenom && $login )
{
	...
	if($sconet_id)
	{
		if( DB_STRUCTURE_tester_utilisateur_SconetId($sconet_id,'professeur',$id) )
		{
			exit('Erreur : identifiant Sconet déjà utilisé !');
		}
	}
