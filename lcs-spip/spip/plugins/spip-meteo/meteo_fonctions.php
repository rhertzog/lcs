<?php
	...
	function meteo_ajouter_boutons($boutons_admin) {
		if ($GLOBALS['connect_statut'] == "0minirezo") {
			$boutons_admin['naviguer']->sousmenu['meteo_tous']= new Bouton('../'._DIR_PLUGIN_METEO.'/img_pack/meteo.png', _T('meteo:meteo'));
		}
	...
	function meteo_verifier_base() {
		...
		if (!isset($GLOBALS['meta']['spip_meteo_version'])) {
			...
			if ($version_base < 1.2) {
				spip_query("ALTER TABLE spip_meteo ADD idx ENUM('', '1', 'non', 'oui', 'idx') DEFAULT '' NOT NULL;");
	...
	function meteo_recherche($flux) {
		...
		$nbm = meteo_afficher_meteos(_T('meteo:meteos_trouvees'), $query_meteo_int);
	...
	function meteo_afficher_meteos($titre_table, $requete) {
		...
		return affiche_tranche_bandeau($requete, '../'._DIR_PLUGIN_METEO.'/img_pack/meteo.png', $couleur_foncee, 'white', $tmp_var, $titre_table, false, $largeurs, $styles, 'meteo_afficher_meteos_boucle', array());
	}
	 ...
	 * @author Pierre Basson
	 **/
	function meteo_afficher_meteos_boucle($row, &$tous_id, $voir_logo, $own) {
		global $options;
	...
	function meteo_afficher_previsions($titre_table, $requete) {
		...
		return affiche_tranche_bandeau($requete, '../'._DIR_PLUGIN_METEO.'/img_pack/previsions.png', $couleur_foncee, 'white', $tmp_var, $titre_table, false, $largeurs, $styles, 'meteo_afficher_previsions_boucle', array());
	}
	 ...
	 * @author Pierre Basson
	 **/
	function meteo_afficher_previsions_boucle($row, &$tous_id, $voir_logo, $own) {
