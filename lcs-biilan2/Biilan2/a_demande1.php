...
if (empty($_POST["val"]))
	...
	foreach ( $val as $id ) 		{$req = mysql_query("SELECT id,login,fulleleve,nomeleve,classe,cpt from dmd_tbl where id LIKE '$id' ") or die ("erreur req ".mysql_error());
		...
		while ( $resultat1 = mysql_fetch_array($req))
			...
			if ($_POST["action"]=="ok")                   				{ mysql_query("INSERT INTO bii_tbl VALUES ('','$elogin','$nomeleve','$fulleleve','$cpt','$date','$dbfullname','----')") or die ("erreur requête ".mysql_error());
...
mysql_query("Delete from dmd_tbl where id = '$iddmd' ");
			...
			if ($_POST["action"]=="ko")
				...
				mail($to,$sujet,$message,"Reply-to: $expediteur\nFrom: $expediteur\n");
                                                             ...
                                                             //Cas pas en mesure de valider de la compétence
