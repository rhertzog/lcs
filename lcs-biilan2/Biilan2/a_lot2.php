	...
	while ( $resultat1 = mysql_fetch_array($req1))
		...
		while ( $resultat2 = mysql_fetch_array($req2))
			{ //boucle 2
					...
					$req20 = mysql_query("SELECT login,cpt FROM bii_tbl WHERE (login LIKE '$id' and cpt like '$affich')") or die ("erreur req ".mysql_err());
					...
					if (!$exist)
						...
						mysql_query("INSERT INTO bii_tbl VALUES ('','$id','$eleve','$dbeleve','$cpt','$date','$dbfullname','$discip')") or die ("erreur requête ".mysql_error());					
							$req3 = ("SELECT login FROM resultat_tbl WHERE login LIKE '$id'") or die ("erreur sql ".mysql_error());
							$resultat3 = mysql_query($req3);
							$nbresultat3 = mysql_num_rows($resultat3);
							if ($nbresultat3==0)		// si l'eleve n'est pas ds la base on cree	
								{mysql_query("INSERT INTO resultat_tbl VALUES ('','$id','1')") or die ("erreur requête ".mysql_error());}
							else
								{
								$req4 = mysql_query("SELECT result FROM resultat_tbl WHERE login LIKE '$id'") or die ("erreur sql ".mysql_error());
								$resultat4 = mysql_fetch_array($req4);	
								$val = "1";
								$note = "$resultat4[result]";
								$total=resultat($note,$val);
								mysql_query("Update resultat_tbl Set result ='$total' where login ='$id'");
								}										
...
<table border="0" cellspacing="0" cellpadding="0" align="center">
  <tr><form name="form1" method="post" action="a_lot.php">
