...
<div class="texte"><h1>Ajout d'une discipline</h1><BR></div>
...
if (empty($_POST["discip1"]))	//on vérifie avec empty si les champs sont vides
	{print "<div class=avertissement><br><br><br><br>Veuillez choisir au moins une discipline<br><br><br><br></div>"; die;}
else
	{
	print "<br><br><br><br>";
	foreach ( $discip1 as $disc ) 		{
		$sql = "SELECT discipline,id from disc_tbl where discipline LIKE '$disc' ";
		$res = mysql_query($sql);
		while ( $resultat = mysql_fetch_array($res))
			{
			$id=$resultat["id"] ;
			print "<div class=confirm><br>La discipline <b>$disc</b> est ajoutée</div>";
			mysql_query("UPDATE disc_tbl SET $etab ='1' WHERE id =$id") or die ("erreur requête ".mysql_error());
			mysql_query("INSERT INTO $discip VALUES ('$id','$disc')") or die ("erreur requête ".mysql_error());
			}
		}
	}
