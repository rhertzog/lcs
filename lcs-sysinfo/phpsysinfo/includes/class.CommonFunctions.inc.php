 ...
 * @version   SVN: $Id: class.CommonFunctions.inc.php 232 2009-06-06 13:26:40Z bigmichi1 $
...
class CommonFunctions
{
    ...
    private static function _findProgram($strProgram)
    {
        ...
        if (PHP_OS == 'WINNT') {
            $strProgram .= '.exe';
            $arrPath = explode(';', getenv("Path"));
        } else {
            $arrPath = explode(':', getenv("PATH"));
        }
        if (PSI_ADD_PATHS !== false) {
            $addpaths = explode(',', PSI_ADD_PATHS);
