...
$out_html .= "<option value=\"admin_right.php?id_area=$id_area&amp;room=-1\">".get_vocab('select_all')."</option>\n";
    ...
    if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
    {
        ...
        $out_html .= "<option $selected value=\"$link\">" . grr_htmlSpecialChars($row[1].$temp)."</option>\n";
    }
    $out_html .= "</select></div>\n
       <script type=\"text/javascript\" >
       <!--
...
if ($room != -1) {
    ...
    if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
        {
        $login_admin = $row[0];
        $nom_admin = grr_htmlSpecialChars($row[1]);
        $prenom_admin = grr_htmlSpecialChars($row[2]);
        echo "<b>";
    ...
    if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
    {
        ...
        if ($is_admin=='yes') {
            ...
            echo "<b>";
            echo grr_htmlSpecialChars($row[1])." ".grr_htmlSpecialChars($row[2])."</b> | <a href='admin_right.php?action=del_admin_all&amp;login_admin=".urlencode($row[0])."&amp;id_area=$id_area'>".get_vocab("delete")."</a><br />";
        }
    }
    if ($exist_admin=='no') {
