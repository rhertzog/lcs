...
function mrbsDelEntry($user, $id, $series, $all)
{
    ...
    $sql = "SELECT beneficiaire, id, entry_type FROM grr_entry WHERE ";
    ...
    for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
    {
        ...
        if (grr_sql_command("DELETE FROM grr_entry WHERE id=" . $row[1]) > 0)
            $removed++;
        grr_sql_command("DELETE FROM grr_entry_moderate WHERE id=" . $row[1]);
    }
...
function mrbsOverloadGetFieldslist($id_area,$room_id=0)
{
  if ($room_id > 0 ) {
      ...
      $id_area = grr_sql_query1("select area_id from grr_room where id='".$room_id."'");
      if ($id_area == -1) {
  ...
  if ($id_area == "")
      $sqlstring = "select fieldname ,fieldtype, grr_overload.id, fieldlist, grr_area.area_name, affichage, overload_mail, grr_overload.obligatoire, grr_overload.confidentiel from grr_overload, grr_area
      where(grr_overload.id_area = grr_area.id) order by fieldname,fieldtype ";
  else
      $sqlstring = "select fieldname,fieldtype, id, fieldlist, affichage, overload_mail, obligatoire, confidentiel from grr_overload where id_area='".$id_area."' order by fieldname,fieldtype";
  $result = grr_sql_query($sqlstring);
  $fieldslist = array();
  if (! $result) fatal_error(1, grr_sql_error());
...
//          $overload_array[$field] = base64_decode($data);  //tructruc
        ...
        $overload_array[$field]["affichage"] = grr_sql_query1("select affichage from grr_overload where id = '".$fieldslist[$field]["id"]."'");
        $overload_array[$field]["overload_mail"] = grr_sql_query1("select overload_mail from grr_overload where id = '".$fieldslist[$field]["id"]."'");
        $overload_array[$field]["obligatoire"] = grr_sql_query1("select obligatoire from grr_overload where id = '".$fieldslist[$field]["id"]."'");
        $overload_array[$field]["confidentiel"] = grr_sql_query1("select confidentiel from grr_overload where id = '".$fieldslist[$field]["id"]."'");
      }
...
function mrbsCreateSingleEntry($starttime, $endtime, $entry_type, $repeat_id, $room_id,
                               $creator, $beneficiaire, $beneficiaire_ext, $name, $type, $description, $option_reservation,$overload_data, $moderate, $rep_jour_c)
{
...
//    $overload_data_string .= $begin_string.base64_encode($overload_data[$id_field]).$end_string; // tructruc
   ...
   $sql = "INSERT INTO grr_entry (  start_time,   end_time,   entry_type,    repeat_id,   room_id,
                            ...
                            VALUES ($starttime, $endtime, '".protect_data_sql($entry_type)."', $repeat_id, $room_id,
                                    '".protect_data_sql($creator)."', '".protect_data_sql($beneficiaire)."', '".protect_data_sql($beneficiaire_ext)."', '".protect_data_sql($name)."', '".protect_data_sql($type)."', '".protect_data_sql($description)."', '-', '".$option_reservation."','".protect_data_sql($overload_data_string)."', ".$moderate.",".$rep_jour_c.")";
...
function mrbsGetRepeatEntryList($time, $enddate, $rep_type, $rep_opt, $max_ittr, $rep_num_weeks, $rep_jour_c)
{
    ...
    for($i = 0; $i < $max_ittr; $i++)
    {
        ...
        switch($rep_type)
        {
            ...
            case 6:
                $sql = "SELECT * FROM grr_calendrier_jours_cycle WHERE DAY >= '".$time2."' AND DAY <= '".$enddate."' AND Jours = '".$rep_jour_c."'";
                                   ...
                                   $room_id, $creator, $beneficiaire, $beneficiaire_ext, $name, $type, $description, $rep_num_weeks, $option_reservation,$overload_data, $moderate, $rep_jour_c)
{
    ...
    if(empty($reps))
    {
        ...
        $id_first_resa = grr_sql_insert_id("grr_entry", "id");
        return;
    }
    ...
    if($ent)
    {
        ...
        for($i = 0; $i < count($reps); $i++) {
            ...
            $id_new_resa = grr_sql_insert_id("grr_entry", "id");
            // s'il s'agit d'une modification d'une ressource déjà modérée et acceptée : on met à jour les infos dans la table grr_entry_moderate
...
$room_id = grr_sql_query1("select room_id from grr_entry where id='".$_id."'");
...
$sql = "select repeat_id from grr_entry where id =".$_id;
$res = grr_sql_query($sql);
...
if ($series==0) {
    ...
    if ($_moderate == 1) {
        $sql = "update grr_entry set moderate = 2 where id = ".$_id;
    } else {
        $sql = "update grr_entry set moderate = 3 where id = ".$_id;
    }
    $res = grr_sql_query($sql);
    if (! $res) fatal_error(0, grr_sql_error());
    ...
    foreach ($tab_entry as $entry_tom) {
        ...
        if ($test == 0) {
           ...
           if (!(grr_backup($entry_tom,$_SESSION['login'],$_description))) fatal_error(0, grr_sql_error());           // Backup : on enregistre les infos dans grr_entry_moderate
...
if ($_moderate != 1) {
    ...
    if ($series==0) {
        $sql = "delete from grr_entry where id = ".$_id;
        $res = grr_sql_query($sql);
        if (! $res) fatal_error(0, grr_sql_error());
    } else {
