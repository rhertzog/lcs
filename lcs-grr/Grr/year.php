...
if ($_GET['pview'] != 1) {
	...
	echo '<td><a title="'.grr_htmlSpecialChars(get_vocab('back')).'" href="'.page_accueil('no').'">'.$vocab['back'].'</a></td>';
...
for ($k = 0; $k<$days_in_month; $k++) {
    ...
    if ($display_day[$cweek]==1) {
        ...
        echo "<div><a title=\"".grr_htmlSpecialChars(get_vocab("see_all_the_rooms_for_the_day"))."\"   href=\"day.php?year=$year_num&amp;month=$month_num&amp;day=$cday&amp;area=$area\">$name_day</a>";
...
for ($ir = 0; ($row = grr_sql_row($res, $ir)); $ir++)
{
   ...
   if ($verif_acces_ressource) {  // on n'affiche pas toutes les ressources
    ...
    echo "<tr>";
    tdcell("cell_hours");
    echo grr_htmlSpecialChars($row[0]) ."</td>\n";
    $li++;
    //Corrige un bug avec certains fuseaux horaires (par exemple GMT-05:00 celui du Québec) :
    //plusieurs mois débutent par le dernier jours du mois précédent.
    //En changeant "gmmktime" par "mktime" le bug est corrigé
    ...
    if (est_hors_reservation(mktime(0,0,0,$month_num,$cday,$year_num),$area)) {
        ...
        if ($d[$cday][$cmonth][$cyear]["room"][$i]==$row[0]) {
           ...
           if ($acces_fiche_reservation)
             echo "<a title=\"".grr_htmlSpecialChars($d[$cday][$cmonth][$cyear]["data"][$i])."\" href=\"view_entry.php?id=" . $d[$cday][$cmonth][$cyear]["id"][$i]."&amp;page=month\">"
