...
if ($valid == "yes")
{
    ...
    $sql = "SELECT email, source, nom, prenom FROM grr_utilisateurs WHERE login='".$_SESSION['login']."'";
...
$sql = "SELECT nom, prenom, statut, email, default_area, default_room, default_style, default_list_type, default_language, source  FROM grr_utilisateurs WHERE login='".$_SESSION['login']."'";
$res = grr_sql_query($sql);
if ($res)
{
    for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
