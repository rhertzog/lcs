...
if (grr_sql_count($res) == 0)
{
    ...
    if ($_GET['pview'] != 1)
    {
      ...
      for ($weekcol = 0; $weekcol < 7; $weekcol++)
      {
        ...
        if ($display_day[$num_week_day] == 1) // on n'affiche pas tous les jours de la semaine
            echo tdcell("cell_hours", 12.5)."<a title=\"".grr_htmlSpecialChars(get_vocab("see_all_the_rooms_for_the_day"))."\" href='day.php?year=".$temp_year."&amp;month=".$temp_month."&amp;day=".$num_day."&amp;area=".$area."'>" . get_vocab("allday")."</a></td>\n";
        $num_week_day++;// Pour le calcul des jours à afficher
        $num_week_day = $num_week_day % 7;// Pour le calcul des jours à afficher
      }
      echo "</tr>\n";
