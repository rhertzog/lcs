...
function reporton(&$row, $dformat)
{
  global $vocab, $enable_periods;
    ...
    #Description
    if ($row[4] != "")
        $description = nl2br(grr_htmlSpecialChars($row[4]));
    else
        $description = "&nbsp;";
    ...
    echo "<td>".grr_htmlSpecialChars($row_user[0]) ." ". grr_htmlSpecialChars($row_user[1])."</td>";
...
if (isset($champ[0]))
{
    ...
    while ($k < count($texte)) {
        ...
        $texte_default[$k] = grr_htmlSpecialChars($texte[$k]);
        $k++;
    }
...
if (isset($_GET["is_posted"]))
{
    ...
    if (( !isset($_GET['pview'])  or ($_GET['pview'] != 1)) and (($summarize != 4) and ($summarize != 5))) {
        ...
        <a href="' . traite_grr_url("","y")."report.php" . '?' . grr_htmlSpecialChars($_SERVER['QUERY_STRING']) . '&amp;pview=1" ';
