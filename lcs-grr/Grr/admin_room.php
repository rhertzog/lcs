...
if (grr_sql_count($res) != 0) {
    ...
    for ($i = 0; ($row = grr_sql_row($res, $i)); $i++) {
        ...
        if(authGetUserLevel(getUserName(),$row[0],'area') >= 4) {
            ...
            echo "<td><a href=\"admin_type_area.php?area_id=$row[0]\"><img src=\"img_grr/type.png\" alt=\"".get_vocab('edittype')."\" title=\"".get_vocab('edittype')."\" align=\"middle\" border=\"0\" /></a></td>\n";
    ...
    if(isset($area)) {
        $res = grr_sql_query("select id, room_name, description, capacity, max_booking, statut_room from grr_room where area_id=$area order by order_display, room_name");
        if (! $res) fatal_error(0, grr_sql_error());
        if (grr_sql_count($res) != 0) {
