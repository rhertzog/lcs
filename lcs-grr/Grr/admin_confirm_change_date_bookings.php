...
if (isset($_GET['valid']) and ($_GET['valid'] == "yes")) {
    if (!saveSetting("begin_bookings", $_GET['begin_bookings'])) {
        ...
        $del = grr_sql_query("DELETE FROM grr_entry_moderate WHERE (end_time < ".getSettingValue('begin_bookings').")");
        $del = grr_sql_query("DELETE FROM grr_entry_moderate WHERE end_date < ".getSettingValue("begin_bookings"));
    }
