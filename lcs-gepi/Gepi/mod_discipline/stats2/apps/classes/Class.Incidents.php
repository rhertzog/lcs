...
// On calcule les totaux globaux
  ...
  private function calcule_totaux_globaux() {
    if(isset($this->incidents))$this->liste_incidents=$this->make_liste_incidents($this->incidents);
    if($this->liste_incidents) $this->totaux=$this->get_totaux_selections($this->liste_incidents,$this->sanctions);
  }
...
// On calcule les totaux par classe  
  ...
  private function calcule_totaux_classe($liste_eleves,$liste,$totaux_indiv) {
    ...
    foreach($liste_eleves as $classe=>$eleves) {
      ...
      if(isset($liste[$classe])) {
        foreach($eleves as $eleve) {
          if(isset ($totaux_indiv[$eleve]['incidents'])) $this->totaux_par_classe[$classe]['incidents']=$this->totaux_par_classe[$classe]['incidents']+$totaux_indiv[$eleve]['incidents'];
          if(isset ($totaux_indiv[$eleve]['mesures'])) $this->totaux_par_classe[$classe]['mesures']=$this->totaux_par_classe[$classe]['mesures']+$totaux_indiv[$eleve]['mesures'];
          if(isset($totaux_indiv[$eleve]['sanctions'])) $this->totaux_par_classe[$classe]['sanctions']=$this->totaux_par_classe[$classe]['sanctions']+$totaux_indiv[$eleve]['sanctions'];
          if(isset($totaux_indiv[$eleve]['heures_retenues'])) $this->totaux_par_classe[$classe]['heures_retenues']=$this->totaux_par_classe[$classe]['heures_retenues']+$totaux_indiv[$eleve]['heures_retenues'];
          if(isset($totaux_indiv[$eleve]['jours_exclusions'])) $this->totaux_par_classe[$classe]['jours_exclusions']=$this->totaux_par_classe[$classe]['jours_exclusions']+$totaux_indiv[$eleve]['jours_exclusions'];
        }
      }
