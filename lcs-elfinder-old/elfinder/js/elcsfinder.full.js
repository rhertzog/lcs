				...
				if (self.options.view == 'icons' && data.images && data.current == self.cwd.hash) {
					for (var i in data.images) {
						if (self.cdc[i]) {
							...
							$('div[key="'+i+'"]>p', self.view.cwd).css('background', ' url("'+data.images[i].replace('/public_html','')+'") 0 0 no-repeat');
						}
...
elFinder.prototype.view = function(fm, el) {
	...
	*/
	this.renderIcon = function(f) {
		var str = '<p'+(f.tmb ? ' style="'+"background:url('"+f.tmb.replace('/public_html','')+"') 0 0 no-repeat"+'"' : '')+'/><label>'+this.formatName(f.name)+'</label>';
	...
	open : function(fm) {
		...
		this.exec = function(dir) {
			...
			function openFile(f) {
				...
				window.open(f.url.replace('/public_html', '')||self.fm.options.url+'?cmd=open&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash), false, 'top=50,left=50,'+ws+'scrollbars=yes,resizable=yes');
			}
		}
	...
	info : function(fm) {
		...
		this.exec = function() {
			...
			function info(f) {
				...
				if (f.url) {
					tb += '<tr><td>'+self.fm.i18n('URL')+'</td><td><a href="'+f.url.replace('public_html/','')+'" target="_blank">'+f.url.replace('public_html/','')+'</a></td></tr>';
				}
...
elFinder.prototype.quickLook = function(fm, el) {
	...
	function update() {
		...
		f.tmb && self.ico.css('background', 'url("'+f.tmb.replace('/public_html','')+'") 0 0 no-repeat');
		if (f.url) {
			self.url.text(f.url.replace('/public_html', '')).attr('href', f.url.replace('/public_html', '')).show();
...
elFinder.prototype.quickLook.prototype.plugins = {
	...
	image : new function() {
		...
		this.show = function(ql, f) {
			...
			if (ql.mimes[f.mime] && f.hash == ql._hash) {
				$('<img/>').hide().appendTo(ql.media.show()).attr('src', f.url.replace('/public_html', '')+($.browser.msie || $.browser.opera ? '?'+Math.random() : '')).load(function() {
					t = $(this).unbind('load');
					if (f.hash == ql._hash) { 
	...
	text : new function() {
		...
		this.show = function(ql, f) {
			if (f.hash == ql._hash) {
				ql.ico.hide();
				ql.media.append('<iframe src="'+f.url.replace('/public_html', '')+'" style="height:'+ql.mediaHeight()+'px" />').show();
			}
		}
	},
		...
		this.show = function(ql, f) {
			if (f.hash == ql._hash) {
				...
				// ql.media.append('<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0"><param name="quality" value="high" /><param name="movie" value="'+url+'" /><embed pluginspage="http://www.macromedia.com/go/getflashplayer" quality="high" src="'+url+'" type="application/x-shockwave-flash" style="width:100%;height:'+ql.mediaHeight()+'px"></embed></object>')
					// .slideDown(400);
				var e = ql.media.append('<embed pluginspage="http://www.macromedia.com/go/getflashplayer" quality="high" src="'+f.url.replace('/public_html', '')+'" style="width:100%;height:'+ql.mediaHeight()+'px" type="application/x-shockwave-flash" />'); 
	...
	audio : new function() {
		...
		this.show = function(ql, f) {
			if (f.hash == ql._hash) {
				...
				ql.media.append('<embed src="'+f.url.replace('/public_html', '')+'" style="width:100%;height:'+h+'px" />').show();
			}
		}
	},
	...
	video : new function() {
		...
		this.show = function(ql, f) {
			if (f.hash == ql._hash) {
				ql.ico.hide();
				ql.media.append('<embed src="'+f.url.replace('/public_html', '')+'" style="width:100%;height:'+ql.mediaHeight()+'px" />').show();
			}
		}
	...
	flv : new function() {
		...
		this.test = function(mime, mimes) {
			return mime == 'video/x-flv' && mimes[mime];
		}
