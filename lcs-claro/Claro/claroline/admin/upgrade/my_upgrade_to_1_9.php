        ...
        foreach ( $config_code_list as $config_code )
        {
            ...
            if ( $config->load() )
            {
                ...
                if ( $config->validate($current_property_list) )
                {
                    ...
                    {
                        $output .= 'Echec : ' . $config->backlog->output();
                        $error = true ;
                    }
                ...
                {
                    $output .= 'Echec : ' . $config->backlog->output() . '</li>' . "\n";
                    $error = true ;
                }
...
// on pousuit par la maj de la bdd si pas erreur
if ( !$error )
    {
       ...
       case DISPLAY_RESULT_PANEL :
        ...
        if ( preg_match('/^1.8/',$currentDbVersion) )
        {
            ...
            if ( $nbError == 0 )
            {
                ...
                echo '<p >Maj des tables en  version 1.9 : OK</p>' . "\n";
                clean_upgrade_status();
