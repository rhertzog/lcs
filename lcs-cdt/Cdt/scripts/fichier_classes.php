...
//traitement du fichier texte		
		if (!empty($_FILES["FileSelection3"]["name"]))
			...
			if (($_FILES["FileSelection3"]["size"]>0) && ($_FILES["FileSelection3"]["type"]=="text/plain"))
				...
				//message
				if (count($data)>0) $mess1= "<h3 class='ok'>1. Le traitement du fichier texte s'est terminé avec succès."."<BR></h3>";
				else $mess1= "<h3 class='ko'> Erreur dans le traitement du fichier texte (Fichier erroné)"."<BR></h3>";
				}
				else $mess1= "<h3 class='ko'> Erreur dans l'importation du fichier texte "."<BR></h3>";
				}
		}
...
//Vérification d'une archive existante
	...
	if ($exist==0)
		...
		if (!($result1&&$result2)) $mess2="<h3 class='ko'>  l'achive n'a pu être créée";
		}
	else
		{
		echo "<script type='text/javascript'>";
		echo" if (confirm('Remplacer l\'archive existante ? ')){";	        
		echo ' location.href = "';echo $_SERVER['PHP_SELF'];echo'"+ "?delarch=" + "yes" + "&nom_arch=" + "'.$name_arch.'" ;} else {';
...
if (isset($_GET['delarch']))
	...
	// Créer la requête.
		...
		if (!($result1&&$result2&&$result3&&$result4)) $mess2="<h3 class='ko'>  l'achive n'a pu être créée";
		}
	}
...
if (isset($_GET['vidtab']))
	...
	if ($_GET['vidtab']=='yes')
		...
		$result3 = @mysql_query ($rq3) or die (mysql_error());
		$result4 = @mysql_query ($rq4) or die (mysql_error());
		if ($result1 && $result2 && $result3 && $result4) $mess2="<h3 class='ok'> les tables onglets et cahiertxt ont été vidées";
		else $mess2="<h3 class='ko'> Une erreur s'est produite lors de l'effacement des données";
...
<fieldset>
<legend>
<FONT FACE="Trebuchet MS, sans-serif" color="#009933">  Cr&eacute;ation du fichier &quot;Liste des classes&quot;  &nbsp  
<?
...
************************/
			...
			echo '<H4>La liste des classes sous forme de menu d&eacute;roulant est &eacute;labor&eacute;e à partir d\'un fichier Php.<br> 
			Ce fichier peut être g&eacute;n&eacute;r&eacute; soit : 
			<ul>
			<li> &agrave; partir de l\'annuaire du LCS
					<P>
						<input type="radio" name="size" value="long"> Format long (Classe_LT_2DE1) <br>
						<input type="radio" name="size" value="court" checked> Format court (2DE1)
					...
					<input type="submit" name="test" value="Tester la cohérence" >  du fichier classe avec l\'annuaire pour un bon fonctionnement.
					</H4>';
...
<fieldset>
<legend>
<FONT FACE="Trebuchet MS, sans-serif" color="#009933"> Cryptage </FONT>
</legend>
<DIV ALIGN=LEFT>
<?
echo '<H4> L\'acc&eacute;s au cahier de texte par les parents se fait par un lien avec le nom des classes crypt&eacute;es. Le crytage doit être 
renouvel&eacute; en d&eacute;but de chaque ann&eacute;e pour que les liens de l\'ann&eacute;e pr&eacute;c&eacute;dente ne soient plus valides.
		<P>
			...
			if ( $cle=="ok") echo ' <FONT color="# 006600" > --> La clé a &eacute;t&eacute; chang&eacute;e </font>';
			elseif ( $cle=="ko") echo ' <FONT color="# 660000" > --> Echec du changement de cl&eacute;</font>
		</P></H4>';
?>
</div>
</fieldset>
<fieldset>
<legend>
<FONT FACE="Trebuchet MS, sans-serif" color="#009933"> Visualisation des absences </FONT>
</legend>
<DIV ALIGN=LEFT>
<?
echo '<H4><br> Le carnet d\'absences pour les &eacute;l&egrave;ves et les parents doit &ecirc;tre activ&eacute uniquement si les informations affich&eacute;es sont significatives.
 Cela implique que tous les professeurs saisissent les absences via le carnet d\'absences du Cdt. 
		<P>
			<input type="radio" name="absence" value="0" ';
...
<fieldset>
<legend>
<FONT FACE="Trebuchet MS, sans-serif" color="#009933"> Param&egrave;trage des cr&eacute;neaux horaires   </FONT>
</legend>
<DIV ALIGN=LEFT>
<?
//Affichage du formulaire de paramétrage des créneaux horaires
		...
		if ($index<5)
		{
		echo "<H4>M ".($index+1)." commence à  " ;
		}
		//créneaux de l'après-midi
		if ($index>4)
		{
		echo "<H4>S ".($index-4)." commence à  " ;
		}
...
//affichage du résultat de paramètrage
	...
	for ($h=0; $h<9; $h++) 
		...
		if (isset ($mess3)) echo "<h3 class='ko'>".$mess3;
?>
</div>
</fieldset>	
</FORM>
