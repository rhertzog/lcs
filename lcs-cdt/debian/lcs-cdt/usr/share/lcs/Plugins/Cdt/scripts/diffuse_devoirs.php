...
<HTML>
<HEAD>
	<meta http-equiv="content-type" content="type" con; charset=windows-1252" >
...
if (!isset($_POST['Valider']))
	...
	//sinon, affichage des classes de même niveau 	
		...
		{
		echo '<H4> <Li>Si vous avez un cours compos&eacute; d\'&eacute;l&egrave;ves issus de plusieurs classes,
		 vous pouvez planifier en une fois, ce devoir pour tous vos d\'&eacute;l&egrave;ves.</li><BR>
		<LI>Lorsque vous enregistrerez votre devoir de <B>'.$mat.'</B> en <B>'.$clas_act.'</B> , il sera aussi planifi&eacute;  pour  les
		classes que vous aurez s&eacute;lectionn&eacute;es ci-dessous </li></H4>';
		echo '<BR><TABLE ><TR>';
		for ($loop=1; $loop < count ($data1)+1  ; $loop++)
			{
			echo '<TD><H5><input type="checkbox" name="sel_cl[]" value="'.$data1[$loop-1].'" />'.$data1[$loop-1].'&nbsp&nbsp&nbsp</H5></TD>';
			if (($loop %5)==0 ) echo'</TR><TR>';
			}
		echo '</TR></TABLE>';
		echo '<DIV align="center" ><input type="submit" name="Valider" value="Valider la s&eacute;lection" ></DIV>'; 
		echo '<BR><Div class="reserve"> N\'oubliez pas apr&egrave;s avoir Valider la s&eacute;lection, de cliquer sur <B>Enregistrer </B>dans la page "Planning" </div>';
		}
	}
?>
</fieldset>	
